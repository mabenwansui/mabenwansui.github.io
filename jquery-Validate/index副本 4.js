...
/*
  $('form').Validate({
    rule : [
      ['user_name', '用户名', '*, 15-16, cn'],
      ['user_email', '邮箱', '*, 15-16, email'],
      ['user_edu', '学历', '*, 15-16'],
      {
        ele: 'user_name',
        title: '用户名',
        type: '*,15-16,email',
        msg_required: '请填写$title',
        msg: '请正确填写$title',
        func: function(){
...
;(function($, window, undefined) {
  ...
  class Validate {
    constructor(element, options){
      ...
      this._element;
      this._tipsType = this._tipsType()[this.options.tipsType];
      this.bindEvent();
      this.refresh();
    }
    _validateRule(){
    ...
    */
    ...
    scan(data, callback){
      ...
      let _success = function(result){
        that._tipsType.highlight(result, false);
        console.log('success');
      };
      let _fail = function(result){
        console.log('fail');
        failArr.push(result);
        that._tipsType.highlight(result, true);
      };
        ...
        return new Promise((resolve, reject) => {
          ...
          that._runYield(function* (){
            ...
            for(let i=0, len=type.length; i<len; i++){
              ...
              if(!ruleFunc) break;
              result = ruleFunc(v[1]);
              if( result.valid === false ) return reject(result);
            }
    ...
    _attributeToJson(ele){
      ...
      let group = ele.attr('validate-group');
      obj.name = group ? ele.find(`:${group}`) : ele;
    ...
    validate(){
      var that = this;
      this.form.find(`[validate-type]`).each(function(){ that._attributeToJson($(this)) });
      this._ruleSort();
      this.scan(this.options.rule, (failArr) => {
        let result = false;
        if(failArr.length > 0){
          if(this.options.fail) result = this.options.fail.call(this, failArr);
          let element = failArr[0].element;
          element.triggerHandler('focus');
        }else{
          if(this.options.success) result = this.options.success.call(this);
        }
        if(result === true) this.form.trigger('submit', [false]);        
      });
    }
    _tipsType(){
      let that = this;
      return {
        AlertTs : {
          highlight: function({element, msg}, flag){
            flag = flag ? 'addClass' : 'removeClass';
            if(element.is(':checkbox, :radio')){
              element.parent()[flag]('validate-error-text');
            }else{
              element[flag]('validate-error').data('validate-msg', msg);
            }
          },
          showTips: function(element, msg){
            this.lastAlertTsElement && this.hideTips(this.lastAlertTsElement);
            element.AlertTs({
              act: 'hide',
              cssStyle: 'error',
              top: 0,
            ...
            }).AlertTs('show', {content: msg});
            this.lastAlertTsElement = element;
          },
          hideTips: function(element){
            element.AlertTs && element.AlertTs('hide');
          },
          bindEvent: function(){
            ...
            that.form.find(filterType)
              .on(`focus.${pluginName}`, function(){
                ...
                if($this.hasClass('validate-error')){
                  self.showTips($this, $this.data('validate-msg'));
                }
              })
            ...
            //change
            that.form.find('input:not(:button,:submit), textarea').on(`change.${pluginName}`, function(){
              //that.scan($(this));
              that.scan($('textarea'));      
            });
          }
